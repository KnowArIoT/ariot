  ...
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      connection.query(sqlQuery, (err, results, fields) => {
        ...
        let storage = [];
        storage.push({"u1": results[0]});
        storage.push({"u2": results[1]});
        storage.push({"gas": results[2]});
        storage.push({"mag": results[3]});
        storage.push({"gps": results[4]});
        storage.push({"accl": results[5]});
        storage.push({"gyro": results[6]});
        resolve(storage);
      });
    });
  });
