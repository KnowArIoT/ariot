...
const getImageData = (limit = 1) => {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      let sqlQuery = 'select url, miliseconds, time from s3 order by `time` desc limit ' + limit;
      ...
      connection.query(sqlQuery, (err, results, fields) => {
        ...
        sqlQuery = 'select data, miliseconds, time from gps where `time` = \'' + results[0]['time'] + '\'';
        connection.query(sqlQuery, (err, results, fields) => {
          connection.release();
          if(err) {
            reject(err);
          ...
          let storage = [];
          storage.push({"image": queryResults, "gps": results});
          resolve(storage);
        });
      });
    });
